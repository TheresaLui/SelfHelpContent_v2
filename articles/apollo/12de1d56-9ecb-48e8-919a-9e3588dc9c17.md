<properties
  pagetitle="Rate Limiting for Meta data operation in Azure CosmosDB "
  description="This Insight is an existing insight from ASC which evaluates for throttling due to Meta data operation.  "
  service=""
  resource=""
  ms.author="balaks,hecepeda"
  selfhelptype="apollo"
  supporttopicids="7d91ee0e-0c06-d5c8-c1b1-4c1eac14f382"
  resourcetags=""
  productpesids="15585"
  cloudenvironments="public,fairfax,mooncake,blackforest,ussec,usnat"
  disableclouds=""
  articleid="12de1d56-9ecb-48e8-919a-9e3588dc9c17"
  ownershipid="AzureData_AzureCosmosDB"
  resourcerequired="False" />
# Rate limiting in Azure Cosmos DB

<insight>
    <symptomId>CosmosDBMetadataInsight</symptomId>
...
If you haven't provisioned enough RUs to meet your per-second request rates, you can increase the throughput (RUs) through the Azure portal, ARM template, PowerShell, Azure CLI, or the Cosmos DB SDK.
...
Your batch size depends on how many Azure Function instances could be running. 
<br>Calculate it this way:

```

your throughput / max number of instances / 10KB document sizes /5 RU per 1K

```

The result is your batch size.
...
### **Causes of throttling and "429s" with autoscale**
...
* Second, you'll see 492s if you have a hot partition. Hot partitions occur when you have logical partition key value that has a disproportionately higher amount of requests compared to other partition key values. For example, if you partition by "companyName", and most of your operations only write or query on one specific company, for example "Contoso", there is a hot partition on the "Contoso" partition key value.

 To see if you have a hot partition, use [Azure Monitor metrics](https://docs.microsoft.com/azure/cosmos-db/monitor-normalized-request-units). This lets you see normalized use by physical partition. If one or a few physical partitions consistently have much higher use than others, there is a hot partition.

 You will see 429s if the underlying physical partition in which a logical partition key resides exceeds 100% normalized utilization.
