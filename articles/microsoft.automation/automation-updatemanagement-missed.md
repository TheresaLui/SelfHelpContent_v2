...
## **Recommended Steps**
Update Management logs attempts to install updates. [Check the job output](https://docs.microsoft.com/en-us/azure/automation/manage-update-multi#view-results-of-an-update-deployment) for errors. 

### **"Failed to Start"**

* This error is commonly caused by machines which are turned off at the time of the deployment. It can also be caused by network connectivity or a machine that has changed names. The ["Failed to Start"](https://docs.microsoft.com/en-us/azure/automation/troubleshoot/update-management#failed-to-start) troubleshooting guide gives specific steps on how to resolve this issue.
...
* This often happens if machines are configured to get updates from WSUS/SCCM, but WSUS/SCCM have not approved the updates
  * The [Windows Update Agent troubleshooter](https://docs.microsoft.com/en-us/azure/automation/troubleshoot/update-agent-issues) can show if there is a WSUS server configured
