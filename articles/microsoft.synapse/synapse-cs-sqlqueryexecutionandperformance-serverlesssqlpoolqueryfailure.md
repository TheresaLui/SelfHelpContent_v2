...
* If your query fails with the error message "File cannot be opened because it does not exist or it is used by another process" and you're sure the file exists and is not in use by another process, this means Serverless SQL pool can't access the file 

	* If you use AAD login, check whether the `UserIdentity` credential exists. If it exists, check if you have proper rights to access the file. Easiest way is to grant yourself **Storage Blob Data Contributor** role on the storage account you're trying to query. [Visit full guide on Azure Active Directory access control for storage for more information](https://docs.microsoft.com/azure/storage/common/storage-auth-aad-rbac-portal). 

	* For credentials defined on storage path level, make sure the credential is not created on storage level, but at least on the container level 
...
* If your query fails with the error message "This query cannot be executed due to current resource constraints", it means that Serverless SQL is not able to run the query due to resource constraints: 

   * Make sure that data types of reasonable sizes are used. Also, specify schema for Parquet files for string columns as they will be `VARCHAR(8000)` by default. 
