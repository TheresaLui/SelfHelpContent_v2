<properties
	selfHelpType = "generic"
	cloudEnvironments = "public, fairfax, blackforest, mooncake, usnat, ussec"
	ownershipId = "AzureData_SQLDataWarehouse"
	service = "microsoft.synapse"
	resource = "workspaces"
	resourceTags = ""
	productPesIds = "15818"
	supportTopicIds = "32738836"
	displayOrder = ""
	diagnosticScenario = ""
	infoBubbleText = ""
	pageTitle = "Synapse Studio, Azure Portal and Client Tools/Synapse Studio"
	description = "Synapse Studio, Azure Portal and Client Tools/Synapse Studio"
	articleId = "synapse-synapsestudioazureportalandclienttools-synapsestudio"
	authors = "saltug"
	ms.author = "saltug"
/>

# Synapse Studio, Azure Portal and Client Tools/Synapse Studio

### Known issues

* If you have multiple AAD logins that is cached to your browser, opening Synapse Studio from Portal might not complete as your browser might try to use a diffrent AAD account to authenticate you to Synapse Studio system. You might need to log off (or remove) other AAD account that is cached to your browser to complete your login to Synapse Studio.


## **Recommended Documents**

* [Launch Synapse Studio](https://docs.microsoft.com/azure/synapse-analytics/quickstart-synapse-studio#launch-synapse-studio)

