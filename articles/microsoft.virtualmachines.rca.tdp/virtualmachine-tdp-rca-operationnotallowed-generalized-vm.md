...
You also get this error if the VM was captured and then restarted, regardless if `sysprep' has has been run on the VM to gneeralize it. If a generalied VM is mistakenly started, as may be the case with this error, `sysprep` must be run again to remove any personal information that may have been saved.
