...
- Stop all the VMs in the availability set, update them to the desired size, and start them again. This deallocates the VMs in Azure and enables the Azure platform to choose from all available clusters when performing the first deployment request for the Start operation. 
