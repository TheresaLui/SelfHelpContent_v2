...
<!--issueDescription-->
We have detected that the deployment for virtual machine **<!--$vmname-->Virtual machine<!--/$vmname-->** initiated at **<!--$StartTime-->StartTime<!--/$StartTime--> (UTC)** failed and the platform detected that the OS experienced a kernel panic during provisioning. One common cause is that the image has an older Linux kernels installed. Your virtual machine is most likely running an older Linux kernel or drivers.
<!--/issueDescription-->

If redeploying the VM does not resolve the issue, please read these additional documents:
...
*To manual fix non-boot issues related to kernel panics, follow the directions in [this article](https://blogs.msdn.microsoft.com/linuxonazure/2016/10/09/linux-recovery-manually-fixing-non-boot-issues-related-to-kernel-problems/).
