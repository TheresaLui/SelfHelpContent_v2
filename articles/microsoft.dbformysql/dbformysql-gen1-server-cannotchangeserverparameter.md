...
Most users can resolve common issues by using the following information.
...
* If the server parameter you want to update is not listed in the Azure portal, you can optionally set the parameter at the connection level using **init_connect**. See [setting parameter not listed](https://docs.microsoft.com/azure/mysql/howto-server-parameters#setting-parameters-not-listed).
...
* To change the **read_only** parameter for non-read replicas, increase the storage size using the portal as described in [reaching storage limit](https://docs.microsoft.com/azure/mysql/concepts-pricing-tiers#storage).
...
* If you want to migrate the database to Azure, see the [migration instructions](https://docs.microsoft.com/azure/mysql/concepts-migrate-dump-restore/).
