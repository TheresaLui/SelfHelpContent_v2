...
1. In the **Web proxy URL** box, enter the URL in this format: 
   `http://host-IP address` or `FQDN:Port number`. 
    **Note:** HTTPS URLs are not supported.

2. Under **Authentication**, select **None** or **NTLM**. If you enable compute and use IoT Edge module on your Azure Stack Edge device, we recommend that you set **Web proxy authentication** to **None**. NTLM is not supported.
