...
* To deploy virtual machines on your Azure Stack Edge Pro device, use templates or go through the Azure portal:
   - When using templates, to create the VMs with dynamic IP address, leave the `privateIPAddress` value blank in the `CreateVM.parameters.json` parameters file. That would let your VM get an IP address from the local DHCP server. Use this parameters file to then create a VM.
