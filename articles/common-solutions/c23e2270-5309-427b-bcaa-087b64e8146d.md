...
## **Recommended Steps**
  ...
  * **SQL Service does not Start because of Missing TempDB Folders**
   ...
   * **Login Failures for Account "NT SERVICE\\SqlIaaSExtensionQuery"**
     ...
     To resolve, uninstall and [reinstall the resource provider](https://docs.microsoft.com/azure/azure-sql/virtual-machines/windows/sql-vm-resource-provider-register?tabs=bash%2Cazure-cli#lightweight-management-mode). Installing of Fullweight Management Mode for the resource provider requires that you restart the SQL Service. Make sure that you have this as a login in your SQL Server, or that you run the following script:
