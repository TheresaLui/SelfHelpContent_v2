...
Most customers can resolve their SQL Connectivity issues using the recommended documents below.

## **Recommended Steps**

  * **Error:"Cannot Generate SSPI Context"** while connecting to SQL Server

    Connectivity to SQL may fail because of SPN duplication, or incorrectly registered or missing entries. Log in to the VM as **Domain Administrator** and [Run Kerberos Configuration Manager](https://www.microsoft.com/download/details.aspx?id=39046) to fix these SPN issues.
  ...
  *   **Azure Active Directory authentication/connectivity is not supported with Azure SQL VM**
   ...
   * **I forgot sysadmin (sa) password, how do I login to SQL Server Instance** 
     ...
     To gain access to SQL Server, run the following command in the command prompt with Administrator permissions. Replace Domain\login with your domain and that user that you want:
