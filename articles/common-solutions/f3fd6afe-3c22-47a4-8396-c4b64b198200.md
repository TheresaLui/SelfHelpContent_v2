...
3.	Enter details of the rule:
    * Name - A name for the rule. Rule names must begin with a letter or a number, be between 2 and 50 characters, and contain no symbols other than dashes (-) or underscores (_).
    b. State - Enabled or disabled
    c. Reason - Select one of the built-in reasons or 'other' if they don't meet your needs
    d. Expiration date - An end date and time for the rule. Rules can run for up to six months.
