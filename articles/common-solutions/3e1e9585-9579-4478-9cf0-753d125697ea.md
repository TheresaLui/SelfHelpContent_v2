...
* **Troubleshoot connectivity between the Mobility service on the replicating machine and the Configuration Server, and between the replicating machine and a Scale-out Process Server**
   ...
   * Ensure that the Mobility Service software on the replicating machine is in a running state. 
    - On Windows, select **Run** > **services.msc** and validate that the **InMage Scout VX Agent – Sentinel/Outpost** is running. 
   ...
   * Verify that the Mobility service on the replicating machine can connect to the Configuration Server on TCP ports 443 and 9443. If you use a scale-out process server, make sure that it can connect to the Process Server on TCP port 9443.
    ...
    - On Windows, check connectivity to Scale-out Process Server: Log in to the machine and open PowerShell. In PowerShell run `Test-NetConnection <Scale-OutProcessServer IP address> -Port 9443`. The cmdlet should return `TcpTestSucceeded : True` if connectivity exists.
     ...
     - On Linux, check connectivity to scale-out Process Server: Log in to the Linux machine and open shell. Run `telnet <Scale-OutProcessServerIPAddress>:9443`. If connectivity exists, the telnet client will be able to open a connection on port 9443 of the scale-out Process Server.<br>
