...
* **Troubleshoot connectivity between the Mobility service on the replicating machine and the Configuration Server, and between the replicating machine and a Scale-out Process Server**
   ...
   * Verify that the Mobility service on the replicating machine can connect to the Configuration Server on TCP ports 443 and 9443. If you use a scale-out process server, make sure that it can connect to the Process Server on TCP port 9443.<br>
     ...
     **On Windows, check connectivity to Scale-out Process Server:** Log in to the machine and open PowerShell. In PowerShell run `Test-NetConnection <Scale-OutProcessServer IP address> -Port 9443`. The cmdlet should return `TcpTestSucceeded : True` if connectivity exists.<br><br>
