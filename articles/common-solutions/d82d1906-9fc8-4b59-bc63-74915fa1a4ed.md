<properties
  pagetitle="Azure Sentinel - Incidents and investigation  - Need help investigating an incident"
  description=""
  service=""
  resource=""
  ms.author="yaronsahar"
  selfhelptype="Generic"
  supporttopicids="32785997"
  productpesids="16690"
  cloudenvironments="public, fairfax, mooncake, blackforest, ussec, usnat"
  disableclouds=""
  articleid="d82d1906-9fc8-4b59-bc63-74915fa1a4ed"
  ownershipid="Azure_Sentinel" />
# Azure Sentinel - Incidents and investigation  - Need help investigating an incident

## **Recommended Steps**

1. In case of a possible false positive incident:
	1. Consider modifying the analytic rule to exclude similar false positive incidents in the future. You can find a link to the analytic rule that generated the incident on the incident page. To learn about working with Analytic rules click [here](https://docs.microsoft.com/azure/sentinel/tutorial-detect-threats-custom). 
	2. If the issue with an analytic rule template provided by Microsoft, please go to our GitHub repository to check that you are using the latest version of the template or leave a comment directly on GitHub to our research team.

2. In case of a possible true positive incident:
	1. To investigate the threat you can review to following documentation on [investigating incidents](https://docs.microsoft.com/azure/sentinel/tutorial-investigate-cases) and on [responding to threats](https://docs.microsoft.com/azure/sentinel/tutorial-respond-threats-playbook).

## **Recommended Documents**

* [Working with analytic rules](https://docs.microsoft.com/azure/sentinel/tutorial-detect-threats-custom)
* [Sentinel GitHub repository for analytic rule templates](https://github.com/Azure/Azure-Sentinel/tree/master/Detections)
* [Investigating incidents](https://docs.microsoft.com/azure/sentinel/tutorial-investigate-cases)
* [Responding to threats](https://docs.microsoft.com/azure/sentinel/tutorial-respond-threats-playbook)