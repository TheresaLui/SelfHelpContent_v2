...
The Service Accounts running SSIS Service, SQL Server Agent Service, and SQL Server Agent Service require privileges at the Operating System level to execute SSIS packages. Without these permissions, deployment of an SSIS package to SSISDB Catalog, execution of an SSIS package located in SSISDB Catalog, and execution of an SSIS package via SQL Server Agent job may be impacted.
...
SSIS, SQL Server and SQL Server Agent service accounts require the following privileges:
