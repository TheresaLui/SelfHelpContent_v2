...
To cancel the pipeline, you can click on the ‘Cancel’ icon in the ADF UX Monitoring page, or Cancel the pipeline via REST API by following the documentation linked below. 
...
Cancelling a pipeline run can take a variable amount of time, based on the number of activities in the pipeline, and if also recursively cancelling all child pipeline runs. When a pipeline is in ‘Cancelling’ state, no new activities in that pipeline run will start and all ‘Queued’ and ‘InProgress’ activities will be attempted to be cancelled if they have not yet finished. If you see new activities continuing to run, please create a support case. 
