...
2. Incident is missing important information:
	1. If the incident is missing entities, check if the analytic rule contains the required entity mapping configurations and if the fields are mapped correctly to the entities you wish to see. 
	Learn more about [entity mapping and entities in Sentinel](https://docs.microsoft.com/azure/sentinel/identify-threats-with-entity-behavior-analytics#entities-in-azure-sentinel).
		
3. If you can’t navigate to the events that generated the alerts in the incident, go to the event that generated an incident by selecting the **Events** icon on the incident page.
...
4. If you wish to see other details on the incident page that are not currently supported, submit a feature request using [Azure Voice](https://feedback.azure.com/forums/920458-azure-sentinel).
