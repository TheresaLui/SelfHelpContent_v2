...
* **Trigger on Alert creation (When an Azure Sentinel Alert was triggered)**
   ...
   **Note:** Only Scheduled query rules allow setting automated response for alert creation. For other rules, such as Microsoft providers, use incident trigger and Automation rules.
   ...
   - In the Azure Sentinel menu, select **Automation**, and create or edit the automation rule that will trigger this playbook. Make sure to attach the playbook to this rule.
   ...
   - The playbook might not have beene triggered due to a failure of holding the selected conditions. Try removing the conditions in order to see which condition is not properly set.
