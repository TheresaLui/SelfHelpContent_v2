...
Suggested revision:
You can monitor the public IP address of all resources deployed through Azure Resource Manager (classic is not supported). This includes monitoring Azure Load Balancers where the backend virtual machines are in the virtual network. This feature does not apply to Azure App Service Environments. For a complete list, see [Virtual network for Azure services](https://docs.microsoft.com/azure/virtual-network/virtual-network-for-azure-services#services-that-can-be-deployed-into-a-virtual-network).
