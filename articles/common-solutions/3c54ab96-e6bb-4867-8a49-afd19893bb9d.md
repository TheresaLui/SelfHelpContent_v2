...
### **Trouble with `iotedge` command**

Iotedge command connects to the IoT Edge Security Daemon (iotedged) and will also make queries into the container runtime. The user running the tool needs to have permission to connect to the management socket for iotedged.
...
```
  If one or the other reports "Unit \[...\].service could not be found" then please try [reinstalling the edge runtime](https://docs.microsoft.com/azure/iot-edge/how-to-install-iot-edge-linux).
...
```
* Ensure iotedged management URI is the same as configured in daemon. 
