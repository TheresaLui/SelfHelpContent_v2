...
- The recommended way of structuring the namespace is <company>/<org-domain>/<service> e.g. sap/billing/sbsvcfor SaaS scenarios. For non-SaaS the suffix svc is not required e.g.sap/myorgdomain/1

When calling the REST Management API (e.g. Queue creation/deletion) a 403 Forbidden is returned. 
- The **management** option is most probably not set to **true** in the service descriptor json. You need to [add management true to your json](https://help.sap.com/viewer/bf82e6b26456494cbdd197057c09979f/Cloud/en-US/d0483a9e38434f23a4579d6fcc72654b.html) and update the service instance

What AMQP library is used in the samples?
- The library is based on Apache QPID with JMS. The EMJAPI provided is simply a wrapper of that library and helps fetching the token
...
- Please check if the client you are using has "messagingrest" capability turned on (in service descriptor )
- Please also check the "rules" section of service descriptor if publish/consume on this queue/topic is allowed

Why is message consumption API "POST" and not "GET"?
- Because consuming a message changes the state of the queue. Message is deleted from the queue once consumed

Why is my webhook in "temporarily inactive" state?
- This may happen during an unplanned outage or problem with your event bus. This often auto-heals within hours. If your webhook stays in this state for a longer time, please contact us
