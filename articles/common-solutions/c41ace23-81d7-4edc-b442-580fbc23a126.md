...
However, in some scenarios, you need to idemtify the fully qualified name of a specific file that is masked under a Resource Set. In those scenarios, you can browse to the **Resource Set** asset. It will show you the `partitionCount` which represents the number of files/partitions in the Resource Set and `aggregatedProperties` with a `sampleUri` that will pinpoint the fully qualified file name.
