...
For common issues, users were able to resolve their "Log Analytics data export to storage account or event hub" issue using the steps below.

## **Recommended Steps**

* Can't configure export via UI or PowerShell - data export is supported in CLI and REST currently, but we are planning to support configuration in UI and PowerShell
* Table was configured in export rule but no data is exported to destination
   ...
   * Export rules take effect about 15 minutes after configuration and the export operate on new ingested data. If you need to query old data in your workspace, you can [Invoke a one time export](https://www.powershellgallery.com/packages/Invoke-AzOperationalInsightsQueryExport) using a PowerShell script, or [query data using Logic App](https://docs.microsoft.com/azure/azure-monitor/platform/logs-export-logic-app)
   * Verify that 'Microsoft.Insights' [resource provider is registered for your subscription](https://docs.microsoft.com/azure/azure-monitor/platform/logs-data-export?tabs=portal#register-resource-provider)
