...
This article provides resources and solutions that can help resolve most replication issues.

## **Recommended Steps**

Refer to the following links for guidance.

### **Agentless replication of VMware virtual machines**

- [Support matrix and permissions required for agentless VMware migration](https://go.microsoft.com/fwlink/?linkid=2151432)
- [Common questions related to the Azure Migrate: Server Migration tool](https://go.microsoft.com/fwlink/?linkid=2151546)


## **Common issues and solutions**
Refer to this list if you have issues during the replication process. 
...
### Start replication fails with an error
   ...
   Azure Migrate creates a Key Vault to manage access keys to the replication storage account in your subscription. To create the vault, set permissions (**Owner, or Contributor and User Access Administrator**) on the resource group in which the Azure Migrate project resides.
...
* "Key Vault operation failed. Operation is Generate shared access signature definition."
  ...
  These errors happen if the logged-in user doesn't have the necessary permissions to configure storage accounts to be managed by the Key Vault. Give yourself the necessary Key vault permissions by using the following PowerShell snippet:
...
### Replication cycle failed with an error
   "No disk snapshots were found for snapshot replication"**
...
If your issue is not listed here, refer to [this troubleshooting article](https://go.microsoft.com/fwlink/?linkid=2143853).
