...
Azure Resource Mover currently moves resources across regions within the same subscription. If you are looking to move resources across regions and across subscriptions simultaneously. Its a two part move process:
1) Move across regions using Azure Resource Mover
2) Then move across subscriptions using Azure Resource Manager (ARM)
