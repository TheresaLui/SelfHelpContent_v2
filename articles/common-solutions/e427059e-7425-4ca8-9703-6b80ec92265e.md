...
1. If you want to create a new Workbook from scratch:

	1. Make sure that the required data type is connected.
	2. Select Add workbook to create a new workbook.
	3. Learn how to create a workbook from scratch: [Create interactive reports with Azure Monitor Workbooks](https://docs.microsoft.com/azure/azure-monitor/platform/workbooks-overview)
	4. When building a query, make sure the Data source is set to Logs and Resource type is set to Log Analytics, and then choose the relevant workspace(s).

2. If you want to import a workbook json from an external source (Github f.e.):

	1. Select Add workbook to create a new workbook.
	2. Select Edit, and then select Advanced editor.
	3. Replace the existing json code with the json code that you would like to import, and apply changes.
