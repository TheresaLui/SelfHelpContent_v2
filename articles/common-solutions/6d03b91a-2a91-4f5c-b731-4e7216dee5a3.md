...
Learn about data connectors for Azure Sentinel by reviewing the following steps.

## **Recommended Steps**

The Microsoft products data connectors are based on Service-to-Service integrations, which can be enabled by the UI or the API.
...
3. The Office 365 integration is based on a native integration with the Office management API. This connector allows you to ingest SharePoint, Exchange, and Teams audit logs.<br>
   Some common issues include:<br>
   * [Office365 solution retirement in Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/insights/solution-office-365)
   * For a connector to access data through the Office 365 Management Activity API, follow these [steps](https://docs.microsoft.com/azure/sentinel/connect-office-365)
   * Multi-tenant scenarios: The Office 365 native connector in Azure Sentinel allows single tenant integration only. In order to collect data from a multi-tenant environment, we suggest that you use a custom logs integration solution based on the Office Management API. See the following [suggested solution](https://github.com/Azure/Azure-Sentinel/tree/master/DataConnectors/O365%20Data).
   * Connect unsupported Office workloads via the Office Management API. If the Azure Sentinel Office365 data connector doesn't include your desired source (and this source/workload is available in the [Office Management API](https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-schema)) we suggest that you use the [Office custom log solution provided in Azure Sentinel GitHub](https://github.com/Azure/Azure-Sentinel/tree/master/DataConnectors/O365%20Data).
