...
Most customers can resolve issues with exceptions, unexpected termination, and non-responsiveness in SQL Server Analysis Services (SSAS) using the following resources. 
When problems occur, run the [SSASDiag Data Collection Tool](https://github.com/ssasdiag/SSASDiag/releases) to collect data on the Analysis Services server. This data can be used for cause analysis.
