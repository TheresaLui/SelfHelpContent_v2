...
1. Hub-spoke networks (or any other topology of peered networks) can configure a Private Endpoint that connects the main hub VNet and an AMPLS, versus having Private Endpoints on each VNet. This is most suitable if for networks that use shared Azure Monitor resources. However, if you prefer to allow each VNet to access a separate set of monitoring resources, create a Private Link to a dedicated AMPLS for each network.
...
3. The global nature of some Azure Monitor endpoints, a VNet can't access some monitoring resources (for example, Workspaces and Application Insights components) over Private Link, and other over a public connection. Once you connect a VNet to an AMPLS through a Private Endpoint, *all monitoring resources must be connected to an AMPLS. Public access to these resources will not be possible from this VNet.
