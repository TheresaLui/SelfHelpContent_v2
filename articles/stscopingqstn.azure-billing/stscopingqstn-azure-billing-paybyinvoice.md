<properties
	pageTitle="Scoping questions for Pay by InvoiceStorage/Development"
	description="Scoping questions for Pay by Invoice"
	authors="prdasneo"
	selfHelpType="supportTopicBasedScopingQuestions"
	supportTopicIds="32454866"
	productPesIds="15659"
	cloudEnvironments="public"
/>

# Pay by Invoice
Enter the following information
* Company name
* Billing address
* [Account administrator's email address](https://docs.microsoft.com/azure/billing/billing-add-change-azure-subscription-administrator#check-the-account-administrator-of-the-subscription)
