<properties
	pageTitle="Escalate to PTAs"
	description=""
    service="Microsoft.Billing"
    resource="Microsoft.Billing/Subscriptions"
	authors="rimayber"
	ms.author="rimayber"
	displayOrder=""
	selfHelpType="TSG_Content"
	supportTopicIds=""
	resourceTags=""
	productPesIds=""
	cloudEnvironments="public, fairfax, usnat, ussec"
	articleId="20c5a5d5-8e95-40ab-8156-c3d6ba9084ce"
   	ownershipId="ASMS_Billing"
/>

# Escalate to PTAs

<!--issueDescription-->

[TSG for PTA engagement](https://internal.support.services.microsoft.com/en-us/help/2929022Issue) <Include a short description of the issue, preferably three sentences or less.>

Request: <What is the specific request that you have for the TA or PTA?>

Case Details: (Fill in your customer?s details in the following list, as appropriate.)

Tenant ID:
For CSP
CSP Partner Tenant ID:
CSP End-Customer Tenant ID:
Order ID: 
(If not available or applicable just add N/A)
Subscription ID (CMAT):
Subscription GUID: 
(If not available or applicable just add N/A)
For Modern
Asset ID:
For RI Issues
Reservation Order ID/Reservation ID:
Subscription Type:
(EA, Pay-As-You-Go, etc.)
MPN ID:
(If not available or applicable, use N/A.)
EA Enrollment ID:
(If not available or applicable, use N/A.)
TSG(s) referenced: 
(TSG numbers or titles.)
Customer's mood:
Troubleshooting done so far:
Total number of communications:
Executive or Manager escalation?
(Yes/No. If yes, include the email thread as an attachment.)
Partner team(s) engaged:
Status details
Partner engagement date:
Service Level or Agreement level:
Severity:
Last update date from partner date:
Issue Date(s):
Additional details: (Do not paste the customer?s verbatim.)
Attachment(s) to call out:

<Attachment file name>
 

<!--/issueDescription-->

