...
8. If you are Windows Defender installed, please run the following command in elevated (Run as Administrator) PowerShell session and see if issue resolves: `Set-ProcessMitigation -Name <app path> -Disable EAF, IAF` where instead of <app path> place the relevant exe paths to all Office apps (excel.exe, winword.exe, etc.)
