...
A good way to test your policy without affecting your environment is to use [Enforcement Mode](https://docs.microsoft.com/azure/governance/policy/concepts/assignment-structure#enforcement-mode) set to DoNotEnforce or Disabled at assignment time.
Reminder that policy works on an If (this criteria matches) then (take this effect). If you want to deny SQL, the if statement should be written 'if NOT SQL'.
