...
EA billing account admins can review and update cost visibility by going to **Cost Management + Billing** > (select a billing account) > **Policies**. "Department admin can view charges" indicates whether a department admin can see costs for their departments and "Account owner can view charges" indicates whether enrollment account admins and RBAC users can see costs for the specific scopes they have access to. "RBAC users" are those who have Azure RBAC access to a resource group, subscription, or management group.

MCA billing account owners can toggle cost visibility ("View charges") from **Cost Management + Billing** > **Billing profiles** > (select profile) > **Policies**. Billing profile owners will open their billing profile directly from **Cost Management + Billing**. Within MCA, cost visibility can be granularly enabled or disabled per billing profile.
