<properties
	pageTitle="Pricing- Assist me with pricing model"
	description="Pricing- Assist me with pricing model"
	service="azure-billing"
	resource="billing"
	authors="prdasneo"
	ms.author="prdasneo"
	displayOrder="1"
	selfHelpType="generic"
	supportTopicIds="32632933"
	resourceTags=""
	productPesIds="15659"
	cloudEnvironments="Public, Blackforest, Fairfax, Mooncake"
	articleId="pricing-azuresubscriptionsandservices"
	ownershipId="ASMS_Billing"
/>

# azure subscriptions and services

### **Pricing for Azure Services**

To help calculate Pricing details by Product or configure and estimate the costs for Azure Products, refer [Pricing calculator](https://azure.microsoft.com/pricing/calculator/)<br>

### **Understand Billing model**

* To understand your Azure bill refer: [Understand your bill](https://docs.microsoft.com/azure/billing/billing-understand-your-bill)
* Learn how to monitor usage and estimated Costs here: [Monitor usage and estimated costs](https://docs.microsoft.com/azure/azure-monitor/platform/usage-estimated-costs)
* Save on Azure costs using Reserved Instances: [Azure reservations](https://docs.microsoft.com/azure/cost-management-billing/reservations/save-compute-costs-reservations)
* Azure Subscriptions: [Microsoft Azure Offer Details](https://azure.microsoft.com/support/legal/offer-details/)
* Speak with someone from sales about pricing: [Contact Azure Sales](https://azure.microsoft.com/overview/sales-number/)
**Note**: except for monthly fees, Azure usage charges are applicable to all pricing models unless otherwise stated.

For **SendGrid** issues contact support [here](https://support.sendgrid.com/hc/)

Azure customers with an Azure Enterprise Agreement (EA), Microsoft Customer Agreement (MCA), or Microsoft Partner Agreement (MPA) may view and download their pricing in the Azure portal : [Check your billing account type](https://docs.microsoft.com/azure/cost-management-billing/manage/ea-pricing#check-your-billing-account-type)

* **Enterprise Agreement(EA)**
  
  * [View and download EA pricing](https://docs.microsoft.com/azure/billing/billing-ea-pricing#view-and-download-pricing-for-your-microsoft-customer-agreement)
  * [Understand your EA bill](https://docs.microsoft.com/azure/billing/billing-understand-your-bill-ea)
  * [Download usage for EA customers](https://docs.microsoft.com/azure/cost-management-billing/understand/download-azure-daily-usage)
  * [Estimate costs with EA pricing](https://docs.microsoft.com/azure/billing/billing-ea-pricing#estimate-costs-with-the-azure-pricing-calculator)
  
* **Microsoft Customer Agreement (MCA)**
  
  * [Download MCA/MPA pricing](https://docs.microsoft.com/azure/cost-management-billing/manage/ea-pricing#download-pricing-for-an-mca-or-mpa-account)
  * [Understand terms in the price sheet](https://docs.microsoft.com/azure/cost-management-billing/manage/mca-understand-pricesheet)
  * [View and download your Invoice](https://docs.microsoft.com/azure/cost-management-billing/understand/download-azure-invoice)
  * [Track MCA Credit Balance](https://docs.microsoft.com/azure/cost-management-billing/manage/mca-check-azure-credits-balance?tabs=portal)
  
* **Cloud Solution Provider (CSP)**

  * [CSP Offers](https://docs.microsoft.com/partner-center/csp-offers)
  * [Pricing and offers](https://partnercenter.microsoft.com/pcv/sales)
  * [Available Azure services in CSP](https://docs.microsoft.com/partner-center/azure-plan-available)
  
* **Azure Marketplace**<br>
Pricing varies based on product types and publisher specifications. Pricing models include: **BYOL Model, Free, Free Software Trial, Usage-Based, Monthly Fee**

  * Learn more on these models and Azure Marketplace: [Marketplace FAQs](https://azure.microsoft.com/marketplace/faq/)
  * Learn more on Billing for Marketplace service charges: [Understand Marketplace service charges](https://azuremarketplace.microsoft.com/marketplace/apps?search=charges)

## **Recommended Documents**

* **FAQs:**

  * Learn more about service or specific pricing model: [Azure Purchase FAQ](https://azure.microsoft.com/pricing/faq/)
  * Azure Databricks [FAQ](https://docs.microsoft.com/azure/azure-databricks/frequently-asked-questions-databricks)
  * Azure Free Trial [FAQ](https://azure.microsoft.com/pricing/free-trial-faq/)
  * Bandwidth Pricing [FAQ](https://azure.microsoft.com/pricing/details/bandwidth/)
  * Azure Bastian [FAQ](https://docs.microsoft.com/azure/bastion/bastion-overview)
* [Cost Management for Partners](https://docs.microsoft.com/azure/cost-management-billing/costs/get-started-partners)
* Azure Services: [Azure products](https://azure.microsoft.com/services/)
* [Upgrade free subscription to Pay-As-You-Go](https://docs.microsoft.com/azure/billing/billing-upgrade-azure-subscription/)
* [Activate monthly Azure credit for Visual Studio subscribers](https://azure.microsoft.com/pricing/member-offers/msdn-benefits/)
* [Microsoft Partner Network (MPN) - Benefits, requirements, enroll, and manage](https://partner.microsoft.com/membership/core-benefits#Core%20benefits)
* [Getting started with Azure billing and cost management](https://docs.microsoft.com/azure/billing/billing-getting-started)
