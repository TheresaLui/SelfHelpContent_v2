
<properties
	pageTitle="Data upload to Azure"
	description="Learn how to resolve issues related to Data copy failed/completed with errors"
	service="microsoft.databox.jobs"
	resource=""
	authors="sunilrsanjeev"
	ms.author="sunir"
	displayOrder=""
	selfHelpType="generic"
	supportTopicIds="32639201"
	resourceTags=""
	productPesIds="16505"
	cloudEnvironments="public,fairfax, usnat, ussec"
    	articleId="32639185"
	ownershipId="StorageMediaEdge_DataBox"
/>

# Data upload at Azure

Go to the Overview page of the resource. For Data Box orders, if data copy has failed or completed with errors, you can see a list of the files that failed to copy by [reviewing the copy log] (https://docs.microsoft.com/azure/databox/data-box-logs#review-copy-log-during-upload-to-azure)

For Data Box Disk orders, follow the steps here to [review any data upload errors] (https://docs.microsoft.com/azure/databox/data-box-disk-troubleshoot-upload)

## **Recommended Documents**

**Troubleshoot errors during data copy**

* [Data Box](https://docs.microsoft.com/azure/databox/data-box-troubleshoot)
* [Data Box Disk](https://docs.microsoft.com/azure/databox/data-box-disk-troubleshoot)
