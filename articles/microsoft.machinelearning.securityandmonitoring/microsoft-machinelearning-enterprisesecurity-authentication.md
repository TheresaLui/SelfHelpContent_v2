...
Most examples in our documentation use interactive authentication which uses your account in Azure AD to directly authenticate. This can be done using the Azure Machine Learning [Python SDK](https://docs.microsoft.com/python/api/overview/azure/ml/?view=azure-ml-py) as follows: 
...
```
The function call above should prompt you with a UI-based authentication flow.
...
Managed identity authentication can be used to authentication from a virtual machine or compute cluster that is configured with a managed identity. This can be done using the Azure Machine Learning [Python SDK](https://docs.microsoft.com/python/api/overview/azure/ml/?view=azure-ml-py) as follows:
