...
* When multiple roles are assigned to a resource, such as the storage account (required for uploading the backup files from the SMB network share in the MI migration process), the most restrictive role gets applied for the Service Principal, and this would result in "insufficient permissions" error during the validation step. Please grant only Contributor role to the AppID used for the MI migration. Refer to the documentation on [MI pre-requisites](https://docs.microsoft.com/azure/dms/tutorial-sql-server-managed-instance-online#prerequisites) for more information.
