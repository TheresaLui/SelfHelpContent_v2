<properties
	pageTitle="Project or Activity Configuration"
	description="Project or Activity Configuration"
	infoBubbleText=""
	service="microsoft.dms"
	resource="virtualmachines"
	authors="radjaram"
	ms.author="rradjou"
	displayOrder="1"
	articleId="migration-projectconfig-errors"
	diagnosticScenario=""
	selfHelpType="generic"
	supportTopicIds="32673570,32673578,32673579,32673599,32673586,32673590,32673600"
	resourceTags=""
	productPesIds="16307"
	cloudEnvironments="public"
/>

# Migration Project and Activity Configuration

## **Recommended Documents**

* [SQL Server to SQL DB offline](https://docs.microsoft.com/azure/dms/tutorial-sql-server-to-azure-sql#create-a-migration-project)<br>
* [Migrate SQL Server to SQL DB with PowerShell](https://docs.microsoft.com/azure/dms/howto-sql-server-to-azure-sql-powershell)<br><br>
* [SQL Server to SQL DB managed instance offline](https://docs.microsoft.com/azure/dms/tutorial-sql-server-to-managed-instance#create-a-migration-project)<br>
* [SQL Server to SQL DB managed instance online](https://docs.microsoft.com/azure/dms/tutorial-sql-server-managed-instance-online#create-a-migration-project)<br>
* [Custom roles for SQL Server to SQL DB managed instance online migrations](https://docs.microsoft.com/azure/dms/resource-custom-roles-sql-db-managed-instance)<br>
* [Network topologies for Azure SQL DB Managed Instance migrations](https://docs.microsoft.com/azure/dms/resource-custom-roles-sql-db-managed-instance)<br>
* [Migrate SQL Server to SQL DB managed instance with PowerShell](https://docs.microsoft.com/azure/dms/howto-sql-server-to-azure-sql-mi-powershell)<br><br>
* [MongoDB to Azure Cosmos DB's API for MongoDB offline](https://docs.microsoft.com/azure/dms/tutorial-mongodb-cosmos-db#create-a-migration-project)<br> 
* [MongoDB to Azure Cosmos DB's API for MongoDB online](https://docs.microsoft.com/azure/dms/tutorial-mongodb-cosmos-db-online#create-a-migration-project)<br> 
