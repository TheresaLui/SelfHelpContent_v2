<properties
pageTitle="Flow Limit Exceeded"
description="VM network interface is dropping some connections due to the number of flows exceeding the flow limit"
...
ms.author="dbrumley"
displayOrder=""
articleId="HealthModelFlowLimit"
...
<!--issueDescription-->
Microsoft Azure has identified an issue with your VM, **<!--$vmname-->[vmname]<!--/$vmname-->**.  Some connections are being dropped due to flow limit exceeded on the network interface.
<!--/issueDescription-->
...
* Login to the VM and display the IP addresses of active connections (use command `netstat -an` (Windows) or `netstat -l` (Linux))
* If there are a large number of connections, examine the IP addresses to determine if they are legitimate or not
