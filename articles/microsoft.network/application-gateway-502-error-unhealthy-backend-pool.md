...
<!--/issueDescription-->
When all the servers in a back-end pool are unhealthy, Application Gateway won't have any healthy servers to forward the client requests to. In this case, the clients will receive a 502 Bad Gateway error to signify that there are no active servers to accept the requests.
<!--/issueDescription-->

## **Recommended Steps**

If you receive 502 Bad Gateway error, and this occurs because the health probe status of all the servers in your back-end pool are unhealthy, follow the steps below to troubleshoot:
...
2. Based on the error message in the **Details** column, you can troubleshoot the issue and fix the root cause for the probe to be marked unhealthy. The issue could be with your health probe configuration in Application Gateway or your back-end server which doesn't give a healthy response. See [Back-end health troubleshooting](https://docs.microsoft.com/azure/application-gateway/application-gateway-diagnostics#back-end-health) to learn about troubleshooting steps for each error message.
