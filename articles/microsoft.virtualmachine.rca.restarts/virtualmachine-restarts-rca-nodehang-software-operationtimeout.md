<properties
	pageTitle="VMA RCA"
	description="RCA - Node Hang - VM operation timeout"
	...
	ms.author="naterns"
	displayOrder=""
	articleId="UnexpectedVMReboot_Node_Hang_Hardware_Operation_Timeout"
...
The host node reboot was triggered by our Azure monitoring systems that detected a **hardware issue** due to **VM operation timeout** on the physical node where the virtual machine was hosted. This caused your VM to get rebooted. RDP and SSH connections to the VM, or requests to any other services running inside the VM, could have failed during this time.
