<properties
    pageTitle="Have trouble using my RDP session or not responding as expected"
    description="Have trouble using my RDP session or not responding as expected"
    service="microsoft.network"
    resource="bastionHosts"
    authors="spacest"
    ms.author="mariliu"
    displayOrder=""
    selfHelpType="generic"
    supportTopicIds="32641421"
    resourceTags=""
    productPesIds="16757"
    cloudEnvironments="public"
	articleId="bastion-usability-rdp"
/>

# Have trouble using my RDP session or not responding as expected

## **Recommended Documents**

* You can [read](https://aka.ms/bastionhost/copy-paste) the product documentation on how to copy-paste from local device to/from remote session using Azure Bastion.
