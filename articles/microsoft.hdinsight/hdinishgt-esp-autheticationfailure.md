<properties
    pageTitle="Azure HDInsight ESP: Authetication Failure"
    description="TSG / How-to for know scenario"
    service="microsoft.hdinsight"
    resource="clusters"
    authors="omidm1"
    ms.author="omidm"
    displayOrder=""
    selfHelpType="Generic"
    supportTopicIds="32629011"
    resourceTags=""
    productPesIds="15078"
    cloudEnvironments="public, Fairfax, usnat, ussec"
    articleId="hdinishgt-esp-autheticationfailure"
	ownershipId="AzureData_HDInsight"
/>
# Azure HDInsight ESP: Authetication Failure
 
## **Recommended Documents**

* [VNET and Networking](https://docs.microsoft.com/azure/hdinsight/hdinsight-extend-hadoop-virtual-network)
* [On ESP clusters, check password hashes exist in AAD-DS](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-architecture#set-up-different-domain-controllers)
* [On ESP clusters, check if MFA is disabled](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-configure-using-azure-adds#enable-azure-ad-ds)
