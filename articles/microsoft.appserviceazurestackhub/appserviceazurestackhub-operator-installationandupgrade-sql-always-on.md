...
### 2. Verify that SQL Server is reachable from the App Service RP
...
5. At this point, you've determined that SQL Server is not reachable by the App Service RP. If you **did not** deploy SQL Server using the [Quickstart template for Highly Available file server and SQL Server](https://docs.microsoft.com/azure-stack/operator/azure-stack-app-service-before-you-get-started#quickstart-template-for-highly-available-file-server-and-sql-server)), you must use your own process to validate that the SQL instances are healthy, running and available. Then proceed to step 
    ...
    4. First test for connectivity to the SQL Server load balancer, by entering the IP address obtained in step 2.vi in the "Server name" field.  Select the "Connect" button. If login fails and you cannot connect to the SQL Server load balancer, jump to and complete step #vii as you need to proceed with opening a support case.
    5. Now test for connectivity to each SQL Server VM's databases.
        ...
        3. Select the "Connect" button. If login fails and you cannot connect to the SQL Server database, jump to step #vii as you need to proceed with opening a support case. 
    ...
    6. If you were able to successfully connect to the SQL Server load balancer and both VMs, repeat step #4 above to verify whether the App Service RP can reach them. 
...
8. Return to the App Service RP blade. Refresh the page and verify that the database is now reachable. If the database is still not reachable, proceed with opening a support case to contact a support engineer.
