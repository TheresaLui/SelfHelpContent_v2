<properties 
    pageTitle="Server Operations data is incorrect or missing on Failures screen"
    description="Troubleshooting guide for missing/incorrect operations in the Failures triage experience"
...
6. If you see the expected failed request in the results of this query, with an incorrect operation name: Use a [custom Telemetry initializer](https://docs.microsoft.com/azure/azure-monitor/app/api-filtering-sampling#addmodify-properties-itelemetryinitializer) to override the operation name to an appropriate meaningful value.  

7. If you do not see the expected failed request, check to see if sampling is in effect (in that time period). You can use the following Logs query to see the actual sampling rate per type of telemetry item:
