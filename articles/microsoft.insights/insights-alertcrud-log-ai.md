...
1. Review the query to ensure you are using analytics commands compatible with log alert:
    * Alerting service executes the provided analytics query repeatedly at the frequency specified. Hence usage of certain analytics commands with high overhead are not allowed and you can [understand the unsupported commands and alternatives for them](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-log-query) to construct query for log alerts correctly.
