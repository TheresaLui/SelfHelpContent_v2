...
1. Data ingestion time into Log Analytics might be impacting your alert rule:
    ...
    * If the data queried by the alert rule is not yet (fully) ingested into Log Analytics, the alert rule might evaluate the condition not met because of which alert was not triggered
