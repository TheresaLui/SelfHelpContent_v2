...
9. Check to see if sampling is in effect. Use the following Logs query to see the actual sampling rate per type of telemetry item:
   ...
   If you see that `RetainedPercentage` for requests is less than `100`, then a rare request failure may have been sampled out.  Dependencies are sampled in only when the related request is sampled in. To configure sampling from your code for the specific SDK implementation you use and the type of sampling you want to configure, review [Sampling in Application Insights](https://docs.microsoft.com/azure/azure-monitor/app/sampling).
