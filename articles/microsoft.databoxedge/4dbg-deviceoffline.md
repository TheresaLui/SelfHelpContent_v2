...
2. The device may be turned off or paused on the hypervisor.
	...
	1. Your device could have rebooted due to a Windows update. Wait a few minutes and try to reconnect.
	2. In Hyper-V, your device will be paused automatically when the volume that stores snapshots or virtual hard disks, runs out of available storage. The state of the device is listed as *paused-critical* in Hyper-V Manager. Resolve this by creating additional space on the volume.
	3. If you still cannot connect, check the Hyper-V host or ESX server to ensure that the VM is healthy.
