...
### Validate alerts on Virtual Machines

* [Validate alerts for VMs](https://docs.microsoft.com/azure/security-center/security-center-alert-validation)
* [Validate alerts for Windows VM](https://docs.microsoft.com/azure/security-center/security-center-alert-validation#validate-alerts-on-windows-vms-)
* You need to enable command-line arguments auditing. To enable it, use the following command: `reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\policies\system\Audit" /f /v "ProcessCreationIncludeCmdLine_Enabled"`
* In order for EICAR alert to show on the Security Center portal, make sure the resources have the monitoring agent (a.k.a Log Analytics Agent) is installed and sending valid heartbeat. Check here for [Monitoring agent health issues troubleshooting](https://docs.microsoft.com/azure/security-center/security-center-troubleshooting-guide#monitoring-agent-health-issues-).

### Validate alerts on Kubernetes

* Connect to the Kubernetes cluster:  `az aks get-credentials --resource-group myResourceGroup --name myAKSCluster`
* [Validate alerts on Kubernetes](https://docs.microsoft.com/azure/security-center/security-center-alert-validation#validate-alerts-on-kubernetes-)

### Other alert validations
