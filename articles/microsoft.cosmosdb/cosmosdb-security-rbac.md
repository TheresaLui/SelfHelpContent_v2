...
Most users are able to resolve their Role Based Access Control (RBAC) in Cosmos DB issue using the steps below.
...
<br>No. To perform management operations with an Azure AD identity, customer should use Cosmos DB's management plane with Azure RBAC.   


### **I got an error saying that my request got blocked because I don't have the required RBAC permission. How can I solve this?**
<br>This error message explicitly mentions which action Azure Cosmos DB tried to perform. Make sure that the Azure AD identity you have used is associated with a role definition that includes this action.   
