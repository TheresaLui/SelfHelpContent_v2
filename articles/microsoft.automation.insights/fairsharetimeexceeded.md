...
If you would like to execute the runbook in the cloud, you need to optimize the runbook.
##### *Create child runbooks*
If your runbook loops through the same function on a number of resources, such as some database operation on several databases. You can move that function to a [child runbook](https://docs.microsoft.com/azure/automation/automation-child-runbooks) instead of performing the operation serially in the loop. Each of these child runbooks run in parallel decreasing the total amount of time for the parent runbook.
