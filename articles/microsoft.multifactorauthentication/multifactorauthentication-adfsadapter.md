...
1.	Install the Azure Multi-Factor Authentication Server with one of the following configurations: 
  1. Install Azure MFA Server locally on the same server as AD FS 
  2. Install the MFA adapter locally on the AD FS server, and then install the MFA Server on a different computer. In this scenario, you need to install the ADFS adapter on all ADFS servers. 
2.	Run the PowerShell command only from the primary ADFS and restart all the ADFS servers to take effect. 
3.	Make sure the Web Services SDK is installed on MFA server (if MFA server and ADFS are in the same box, it is not necessary). Web service SDK is an IIS component. IIS needs to be installed prior to installing the Web service SDK. 
4.	If you have issue installing the ADFS adapter, make sure that the KB2919355 and .Net framework 4.x is installed. 
