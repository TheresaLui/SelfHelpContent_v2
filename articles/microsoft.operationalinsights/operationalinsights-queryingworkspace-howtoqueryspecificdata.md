...
Querying data can be done in two ways - directly querying a specific table, or searching through your entire set of data. <br/>
The first approach is preferable, since it's scoped to a relevant data source and therefore more efficient.
...
1. Querying a specific table
   ...
   * Get the latest logs<br/>
   To get the latest records, use the *top* operator. For example:<br/>
...
2. Searching through your data set <br/>
   If you don't know which table to query, or intentionally want to query across all tables - you can run an open search query:<br/>
   ``` search "error"```
   The query returns every log records the contains the term "error". <br/>
