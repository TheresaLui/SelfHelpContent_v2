<properties
	...
	infoBubbleText="The IP address is not whitelisted for sending requests to account."
...
<!--issueDescription-->
We found that there is a firewall rule on the account which is blocking access for one or more IP addresses
<!--/issueDescription-->
...
It appears that the current account has an IP access control policy which is restricting access to an IP range. Please follow this [link](https://docs.microsoft.com/en-us/azure/cosmos-db/firewall-support) for to update the IP access control policy on this account
