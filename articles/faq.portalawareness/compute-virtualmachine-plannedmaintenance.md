...
No, the App Service team is rolling out a routine update to the underlying infrastructure of Azure App Services. This will not impact IaaS VMs, Cloud Services, or any other Azure services, and is not associated with the Planned Host OS maintenance. <br>
We have since sent a follow up notification to customers' management portals to clarify that this is specific to app services and functions. During this update, there will be a brief restart of customer web apps, similar to what occurs during the regular monthly OS update.

## How long will the reboot take?
We estimate completion with 30 - 45 minutes.

# Should the guest OS to be updated?
No, not for this issue. The work done on the Host OS will mitigate for this vulnerability. But we always recommend that customers maintain the latest patch levels. Customers should check with their OS provider for guidance. Guidance on Windows Server VM updates is available [here](https://portal.msrc.microsoft.com/en-US/security-guidance/advisory/ADV180002)

## Will a manual reboot or stop deallocate avoid the need for planned maintenance?
Reboots of the guest OS will not eliminate the need for maintenance. Redeploying a VM may ‘land’ on an updated host. Wherever possible, the Azure fabric will try to put new VMs on hosts that are already updated – but attempting this proactively introduces risk. It is entirely possible that redeploying a VM could land on a non-updated host, in which case the customer may have to endure downtime again during their scheduled maintenance. As a result, this is not a practice that we recommend and risks increased customer impact.
