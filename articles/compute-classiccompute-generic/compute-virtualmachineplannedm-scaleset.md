...
Information related to planned maintenance is provided only during a planned maintenance wave. For Azure Virtual Machine Scale Sets, the planned maintenance performs updates to improve the reliability, performance, and security of the host infrastructure for the VMs.
...
- Reboot not required
 Azure uses in-place migration to pause the VM while the host is updated. These operations are applied across fault domains. Operations are stopped if any warning health signals are received.<br>

- Reboot required
 You will receive a notification that shows when a planned maintenance wave is planned. You are given a time window in which you can determine which VMs are included in the wave and the start time.<br>

If you are considering performing self-service maintanence, be aware that self-service maintenance might not be available for all your VMs and is not recommended for deployments using availability sets.<br>
