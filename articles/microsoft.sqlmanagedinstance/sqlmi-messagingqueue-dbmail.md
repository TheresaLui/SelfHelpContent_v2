...
- Check that the options **Database Mail XPs** and **show advanced options** are enabled in [sys.configurations](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-configurations-transact-sql) view.
...
- Try to reach the mail server from a Managed Instance using SQL Agent Job that tests the network connection:
   ...
   3. Check that you have enabled the port that communicates with the email server. Add the Port in the [Outbound security rules](https://docs.microsoft.com/azure/virtual-network/tutorial-filter-network-traffic#create-security-rules) of the Network Security Group that controls access to your Managed Instance.
