...
- Check are you using [supported syntax for Database Mail](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-transact-sql-information#database-mail-db_mail). Windows Authentication and attachments are not supported and there is a known issue with **@quey** parameter.
...
- Check you can reach the mail server from Managed Instance usinf SQL Agent Job that tests the network connection:
