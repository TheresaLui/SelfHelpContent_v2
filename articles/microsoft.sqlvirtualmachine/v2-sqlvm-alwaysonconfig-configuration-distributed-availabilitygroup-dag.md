...
* **Things to Keep in Mind when Configuring Always on Distributed Availability Groups**
  ...
  * In a Distributed AG Setup, WSFC clusters that host the individual availability groups can have different [major versions of Windows Server](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/distributed-availability-groups?view=sql-server-ver15#windows-server-versions-and-distributed-availability-groups). The major versions of SQL Server must be the same.
