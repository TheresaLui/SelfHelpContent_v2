...
**Detection: If you are unaware of the process driving high CPU consumption, you can [download PerfInsights](https://docs.microsoft.com/azure/virtual-machines/troubleshooting/performance-diagnostics#install-and-run-performance-diagnostics-on-your-vm)**. PerfInsights generates a report that contains a dedicated tab for CPU analysis.  It will list out the processes per Average CPU consumption in descending order. It will show point out if the process was a System or related to Microsoft owned App (SQL, IIS) or a Third-Party process. We have a dedicated Sub-Tab under CPU which could be leveraged for detailed pattern analysis, per core, or per process.
...
If you are unable to access your virtual machine via RDP or SSH you can try restarting to temporarily regain access, then proceed to troubleshoot using [PerfInsights](https://docs.microsoft.com/azure/virtual-machines/troubleshooting/performance-diagnostics#install-and-run-performance-diagnostics-on-your-vm) to determine what is causing the CPU spike.
