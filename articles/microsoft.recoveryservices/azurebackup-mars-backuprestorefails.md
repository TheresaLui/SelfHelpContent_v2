...
- If the vault credentials provided are different from the vault with which this server is registered, the Target and Source machines must both be registered with the **same Recovery Services vault** and use the **same Vault Credentials key** to recover the data to different machine.
- Make sure that the target VM's operating system version must be newer or the same as the source VM's operating system version for successful restore. This is a known limitation.
