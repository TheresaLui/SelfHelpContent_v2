...
- Adding a new disk or removing an existing disk can cause the "resynchronization required" alert to be triggered. 
    ...
    - If any of the above is not possible, disable and re-enable protection for the machine. Follow these steps: Navigate to Azure portal > **Recovery Services Vault** > **Open Replicated Items** and select the concerned machine's overview. Then, select **Disable replication**. Once it is disabled, you can enable replication again.
