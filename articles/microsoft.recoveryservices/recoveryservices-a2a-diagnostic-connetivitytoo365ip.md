...
1. Azure Site Recovery requires access to Office 365 IP ranges for authentication. You can sign in to the virtual machine and check if you can access "https://login.microsoftonline.com". 

2. If you are using Azure Network security group (NSG) rules or firewall proxy to control outbound network connectivity on the virtual machine, make sure that you allow communication to Office 365 IP ranges. To do this by using NSG, follow these steps: 
