...
- Cannot find the files/disk to restore?, check unsupported restore configurations to see if the disks are backed up:***Temp drive***, ***Deduplicated Disk***, ***Shared disk*** and ***disk with write Accelerator enabled***. [Learn more](https://docs.microsoft.com/azure/backup/backup-support-matrix-iaas#vm-storage-support)
...
- Special restore configuration requirements for [Dynamic disks, LVM/RAID arrays](https://docs.microsoft.com/azure/backup/backup-azure-restore-files-from-vm#lvmraid-arrays-for-linux-vms)
