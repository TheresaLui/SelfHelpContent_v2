...
1. Consider the size and activity of the database:
   - As expected, the bigger the database, the longer the restore will take
   - Reduce non-clustered indexes that aren't being used, or that can be merged with another non-clustered index
   ...
   - Avoid frequent index maintenance. Work only on the indexes that really require maintenance.
 
2. Consider the service tier of the target Azure SQL Database:
...
3. Avoid long-running transactions:
   ...
   - If the point in time provided has high number of active long-running transactions, these transactions must be rolled back

4. Select the same Azure region as the original Azure SQL Database or paired region:
   - This will prevent the restore data from being sent through the network to another Azure region
...
5. Consider the number of concurrent restore requests being processed in the target region:
...
1. Confirm that the service tier selected as the target can handle the storage needed for the Azure SQL Database
2. When restoring to an Elastic Pool, confirm that it has enough storage available for the database being restored
3. Consider restoring the Azure SQL Database as a single database and then moving it to the Elastic Pool
