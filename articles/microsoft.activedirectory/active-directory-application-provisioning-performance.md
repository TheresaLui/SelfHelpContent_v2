<properties
	...
	supportTopicIds="32629761,32629807"
	productPesIds="16666"
...
The Azure AD user provisioning service perpetually synchronizes user accout data from Azure AD to target applications, typically on 40 minute sync cycle intervals. However, the time it takes to complete a sync cycle depends on what type of cycle it is (initial sync or incremental sync), and how much user data needs to be processed.

A provisioning job can also go into a "quarantine" state, which means that the frequency of sync execution has been reduced due to a high volumne of errors.
...
For recommended steps dealing with slow provisioning jobs and quaratine, see the appropriate sections below.
...
## **Recommended steps for quaratined provisioning jobs**

1. For information on why provisioning jobs can go into Quaratine, see [What happens during automatic provisioning?](https://docs.microsoft.com/azure/active-directory/manage-apps/user-provisioning#what-happens-during-provisioning).

2. If the provisioning job has gone into quaratine due to a high volumne of errors (error code = entryLevelError), check the [provisioning audit logs](https://docs.microsoft.com/azure/active-directory/manage-apps/check-status-user-account-provisioning#provisioning-audit-logs) to view the details of these errors.
