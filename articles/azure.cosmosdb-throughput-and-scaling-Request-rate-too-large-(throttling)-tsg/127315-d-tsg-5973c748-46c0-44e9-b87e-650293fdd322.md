...
* Observe the following chart and  identify if there’s data + index usage skew and the involved partition key range and partition key.
  ...
  <br>b)”Data + Index storage consumed by top partition keys”  <- Help to identify if there’s data + index usage skew and the involved partition keys
  <br>c)“Data + Index storage consumed per partition key range” <- Help to identify where the partition key range the involved partition key belong to.
