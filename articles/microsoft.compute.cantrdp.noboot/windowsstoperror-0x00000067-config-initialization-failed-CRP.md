...
authors="ram-kakani"
displayOrder=""
articleId="windowsstoperror-0x00000067-config_initialization_failed_crp"
...
We have investigated and identified that your VM (VM) <!--$vmname-->[vmname]<!--/$vmname--> is currently in an inaccessible state because windows failed to boot with error code **0x00000067**. This issue occurs when the Initial Machine Configuration (IMC) reference is setup on the Boot loader but its reference is missing in the registry.
