...
4. Open an elevated command prompt (run as administrator) and execute the following:
	...
	2. Get a memory dump sample from the hang process *TrustedInstaller.exe* by entering `procdump.exe -s 5 -n 3 -ma <PID HERE>`.

		> Replace `<PID HERE>` with the PID identified earlier.

	3. Now kill the current process to unlock the shutdown process by entering `taskkill /pid <PID HERE> /t /f`.
