...
3. <!--/Phase 2 Mitigation--> Verify the OS partition that holds the BCD store for the disk is marked as active:
    ...
    b. List the disks on the system, look for added disks, and select the new disk.  To list the disk: ''list disk'' To select the disk: ''sel disk 1''
    c. List all the partitions on the disk and select the partition you want to check. Typically, System Managed partitions are around 350Mb in size. To list the partitions: ''list partition'' To select the partition:''sel <PARTITION ID>''.
    d. Check the status of the partition to verify that it's Active: ''detail partition''
    e. Check the flag called ''Active'' and if it states that isn’t active, change this flag and activate this partition by running ''active''
    f. To validate the changes where done properly, re-run ''detail partition''. Type ''Exit'' when ready.
