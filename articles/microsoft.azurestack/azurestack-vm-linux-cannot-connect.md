...
Most users can resolve Azure Stack Linux VM connectivity issues by using the following information.

## **Recommended Steps**

**I have an issue with my public IP**

By default, Network Security Group denies connections from the internet unless they are explicitly allowed. To resolve this issue, try one or more of the following steps.
...
- If there is no rule to allow traffic from the internet, add a new rule to allow access to the backend port, with source being `internet`, `*`, or a specific public IP address
...
- Connect to the Azure portal, and use the [reset password feature](https://docs.microsoft.com/azure/virtual-machines/troubleshooting/troubleshoot-ssh-connection#use-the-azure-portal) to reset the SSH configuration and the credentials for the user
