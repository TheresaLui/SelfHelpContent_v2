...
* **Azure Stack Admin REST API** for each Resource Provider that ships with Azure Stack, for performing operations on resource objects such as Scale Unit Nodes, Alerts, Updates, Backup, Marketplace, Subscriptions, Offers, and more.<br>
* **Resource provider APIs** for each resource provider [supported by profiles in Azure Stack](https://docs.microsoft.com/azure/azure-stack/user/azure-stack-profiles-azure-resource-manager-versions). Each client uses an API profile to contact the correct resource provider and API version for Azure Stack.<br>
* **Azure CLI** command-line tool for managing Azure Stack resources, designed to enable scripting, querying data, performing long-running operations, and more.<br>
