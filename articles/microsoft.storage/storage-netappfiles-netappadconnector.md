...
1. Note that only one Active Directory connection is allowed in a region. However if "Map multiple accounts to AD server" feature is used then the AD connection will be visible in all NetApp accounts that are under the same subscription and same region. 
