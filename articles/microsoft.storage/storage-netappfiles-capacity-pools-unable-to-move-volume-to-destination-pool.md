<properties
	pageTitle="Unable to move volume to destination Pool"
	description="Unable to move volume to destination Pool"
	service="microsoft.storage"
	resource="storage"
	authors="b-mayada"
	ms.author="b-mayada"
	displayOrder=""
	articleId="UnableToMoveVolumeToDestinationPool"
	selfHelpType="generic"
	supportTopicIds="32777917"
	resourceTags=""
	productPesIds="16469"
	cloudEnvironments="public,MoonCake,FairFax,BlackForest, usnat, ussec"
	ownershipId="AzureNetAppFiles"
/>

# Unable to move volume to destination Pool

## **Recommended Steps**

* [Troubleshoot capacity pool issues](https://docs.microsoft.com/en-us/azure/azure-netapp-files/troubleshoot-capacity-pools)

## **Recommended Documents**

* [Dynamically change the service level of a volume](https://docs.microsoft.com/en-us/azure/azure-netapp-files/dynamic-change-volume-service-level)