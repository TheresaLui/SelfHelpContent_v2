...
<!--issueDescription-->
Server endpoint deletion failed under the Storage Sync Service resource  _**<!--$storageSyncServiceName-->[storageSyncServiceName]<!--/$storageSyncServiceName-->**_ due to error **ECS\_E\_MGMT\_SERVER\_JOB\_EXPIRED** (error code: **-2134347757** or **0x80c87013**). This error occurred between **startTime** and **endTime**.
...
ECS_E_MGMT_SERVER_JOB_EXPIRED

## **Recommended steps**

To delete the server endpoints, follow the steps that are described in [Unregister a server with Azure File Sync](https://docs.microsoft.com/en-us/azure/storage/files/storage-sync-files-server-registration#unregister-the-server-with-storage-sync-service).
...
Removing a server endpoint from a sync group
