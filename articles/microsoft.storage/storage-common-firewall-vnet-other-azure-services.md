...
Most customers resolved their firewall & virtual network related connectivity failure issues, from other Azure Services, using the steps below.

### **Trusted Microsoft services is added to the exception still storage connections get blocked**
 
Only a subset of Microsoft Azure services are part of the [Trusted Services](https://docs.microsoft.com/azure/storage/common/storage-network-security#trusted-microsoft-services) list. Please refer to the list to learn about the services covered under trusted services. 

Any service not on the list will not work until the public IP or VNet of that service is explicitly added to the exception list. Refer to the steps below to learn how to resolve the issue for the most commonly reported services. 
...
* ExpressRoute: Given two IP addresses at the Microsoft Edge that are used to connect to Microsoft Services like Azure Storage. To allow communication from your circuit to Azure Storage you must configure the storage account's 'Firewall and virtual networks' configuration to add those two IP addresses on your storage account. Please open a support case under Networking to get the two IPs. 
...
* [Automation](https://docs.microsoft.com/azure/automation/automation-hybrid-runbook-worker)
	
**Intelligence and Analytics**

* [Data Factory](https://docs.microsoft.com/azure/data-factory/connector-azure-blob-storage#supported-capabilities)
