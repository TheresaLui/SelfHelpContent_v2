...
Azure Database for PostgreSQL - Single server has error, query, and audit logs available. Use query and error logs to identify, troubleshoot, and repair configuration errors and sub-optimal performance. Go to **Concepts** > **Monitor and tune** > **Audit logs** in documentation for guidance on audit logs.
...
* Are you getting started with logging in Azure Database for PostgreSQL - Single server? 
  ...
  * Consider performance consequences of logging. Verbose logging will cause a significant performance overhead. We recommend you only use statement logging parameters like `log_statement` and `log_min_duration_statement` for short periods of troubleshooting. 
  * Note that access to transaction logs is not available.
...
* Are you looking for information on log rotation?
  * The .log files rotate every hour or when it reaches 100 MB in size, whichever comes first
* Are you troubleshooting or tuning log retention?
...
* Are you looking for information on how to configure diagnostic settings?
...
* Are you trying to configure Diagnostic settings for Basic tier?
