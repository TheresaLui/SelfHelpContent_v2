...
# Automated backups
Azure Database for PostgreSQL - Flexible Server performs automated backups of your database server. It also lets you perform point-in-time restore of your server. The following information will help you resolve the most common issues regarding backup and restore.
...
* **Can I choose not to enable backup for my server, because that is for testing only?** <br>
No. All servers are configured to have backups taken.

* **Is the backup performed at the server-level or the database-level?** <br>
The backup is performed at the server level, including all the databases. You cannot choose to backup specific database. You can always use `pg_dump` to back up only the database.
...
Yes. The backup data by default is stored in zone-redundant mode. Therefore, you can to restore the server in a different AZ within that region.
...
You can perform the PITR to a new server. Then, from your PostgreSQL client or using AZ CLI from the portal, you can do a dump of the database or the table and import into your server.
...
* Monitor for backup storage usage. 
