<properties
    pageTitle="Azure Logic Apps - X12 encoding"
    description="Azure Logic Apps - X12 encoding"
    service="microsoft.logicapps"
    resource="logicapps"
    authors="v-miegge"
    ms.author="kawilson"
    selfHelpType="generic"
    supportTopicIds="32780475"
    resourceTags=""
    productPesIds="1004053"
    ownershipId="Compute_LogicApps"
    cloudEnvironments="public, Fairfax, usnat, ussec"
    articleId="1b2cf608-fe93-427f-b8c3-1615a2abdc12"
/>

# Azure Logic Apps - X12 encoding

## **Recommended Steps**

With the Encode X12 message connector, you can validate EDI and partner-specific properties, convert XML-encoded messages into EDI transaction sets in the interchange, and request a Technical Acknowledgement, Functional Acknowledgment, or both.

- [Encode X12 messages in Azure Logic Apps with Enterprise Integration Pack](https://docs.microsoft.com/azure/logic-apps/logic-apps-enterprise-integration-x12-encode?WT.mc_id=Portal-Microsoft_Azure_Support)
- [X12 Encode details](https://docs.microsoft.com/azure/logic-apps/logic-apps-enterprise-integration-x12-encode?WT.mc_id=Portal-Microsoft_Azure_Support#x12-encode-details)

## **Recommended Documents**

- [Getting Started with Logic Apps - XML to EDI X12](https://www.tech-findings.com/2019/11/getting-started-with-logic-apps-xml-to-EDI-x12.html)
- For Azure Logic Apps questions, visit the [Azure Logic Apps forum](https://social.msdn.microsoft.com/forums/home?forum=azurelogicapps)
- To submit or vote on feature ideas, visit the [Logic Apps feedback page](https://feedback.azure.com/forums/287593-logic-apps)
