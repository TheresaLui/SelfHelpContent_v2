...
Please check the application connection string contains the correct username.

Additionally, if you are using a SQL login, check that the login exists:
...
2. Run the following query:
    ...
    ```
    select * from sys.sql_logins where name = 'insert the username here'
    ```
...
2. Run the following query:
    ...
    ```
    select * from sys.database_principals where name = 'insert the username here'
    ```

3. If the query returns no record, please create the user.

For more information about creating logins and users in Azure SQL Database, please refer to [Create SQL Login and SQL User on your Azure SQL DB – Azure SQL Database Support](https://blogs.msdn.microsoft.com/azuresqldbsupport/2016/10/05/create-sql-login-and-sql-user-on-your-azure-sql-db/).
