...
Before the scale operation failovers from the source to the target performance tier, it kills any active transactions on the source instance, to avoid long recovery on the target instance. Recovery is a database process that occurs when a database is restarted after a shutdown – which happens after a failover when the target instance starts up during a scale operation. Recovery rolls back all inflight but uncommitted transactions at the time of restart, and during this time the database is unavailable for additional requests. If there are any large transactions to rollback, this activity takes time proportional to the number of the active transactions. We recommend terminating any open transactions on your database to avoid long recovery after failover and speed up the scale operation. Please refer the links below for related information.
